...
from .tools import (
    LANGUAGE_MAP,
    detect_language, node_to_dict, languages,
    init_parsers
)
...
def register_enhanced_tools(mcp_server):
    ...
    def parse_to_ast_incremental(
        ...
        if old_code:
            ...
            old_result = parse_code_to_ast_incremental(old_code, language=language, filename=filename)
    ...
    def generate_enhanced_asg(
        ...
        """
        ast_data = parse_to_ast_incremental(code, old_code=None, language=language, filename=filename)
    ...
    def diff_ast(
        ...
        """
        ast_old = parse_to_ast_incremental(old_code, language=language, filename=filename)
        ast_new = parse_to_ast_incremental(new_code, language=language, filename=filename)
