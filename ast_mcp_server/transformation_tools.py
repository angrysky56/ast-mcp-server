...
from .tools import detect_language, LANGUAGE_MAP
...
class AstGrepTransformer:
    ...
    def _check_ast_grep_available(self) -> bool:
        ...
        try:
            ...
            logger.warning("ast-grep not found in PATH. Install with: pip install ast-grep-cli")
            return False
        ...
        try:
            ...
            if result.stdout.strip():
                ...
                for match_data in search_results:
                    match = PatternMatch(
                        start_line=match_data.get("range", {}).get("start", {}).get("line", 0),
                        end_line=match_data.get("range", {}).get("end", {}).get("line", 0),
                        start_column=match_data.get("range", {}).get("start", {}).get("column", 0),
                        end_column=match_data.get("range", {}).get("end", {}).get("column", 0),
...
def register_transformation_tools(mcp_server):
...
def create_transformation_report(result: TransformationResult) -> str:
    ...
    report = []
    report.append(f"✅ Pattern transformation completed")
