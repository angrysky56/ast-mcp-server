...
def build_parsers():
    ...
    try:
        import subprocess
        result = subprocess.run(["python", "build_parsers.py"], capture_output=True, text=True)
...
def status():
    ...
    try:
        ...
        if init_parsers():
            table.add_row("Tree-sitter Parsers", "✅ Available", "All language parsers loaded")
        else:
            table.add_row("Tree-sitter Parsers", "⚠️ Limited", "Run 'build-parsers' command")
    except Exception:
    ...
    try:
        ...
        table.add_row("Enhanced Tools", "✅ Available", "Advanced AST/ASG analysis enabled")
    except ImportError:
    ...
    try:
        ...
        table.add_row("Transformation Tools", "✅ Available", "ast-grep integration enabled")
    except ImportError:
...
def test_pattern(
    ...
    code: Optional[str] = typer.Option(None, "--code", "-c", help="Test code (or use minimal test)")
):
